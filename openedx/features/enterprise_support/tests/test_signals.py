...
class EnterpriseSupportSignals(SharedModuleStoreTestCase):
    ...
    def _create_enrollment_for_refund(self, no_of_days_placed=10, enterprise_enrollment_exists=True):
    ...
    def test_refund_order_voucher(self, skip_refund, enterprise_enrollment_exists, no_of_days_placed, api_called):
        ...
        enrollment = self._create_enrollment_for_refund(no_of_days_placed, enterprise_enrollment_exists)
    ...
    def test_refund_order_voucher_with_client_errors(self, mock_error, log_level):
        ...
        enrollment = self._create_enrollment_for_refund()
            ...
            with LogCapture(LOGGER_NAME) as logger:
                ...
                logger.check(
                    (
                        LOGGER_NAME,
                        log_level,
                        'Encountered {} while requesting to create refund voucher form '
                        'the ecommerce. Order=EDX-000000001'.format(mock_error.__name__),
                    )
                )
